...
public class CANdleManager {
    ...
    public CANdleManager(String canbus, Color8Bit positiveY, Color8Bit positiveX, Color8Bit negativeY, Color8Bit negativeX, CANdleConstants ... candles)
    {
        for(var constants : candles)
        {
            m_list.add(new CANdleInternalConstants(new CANdle(constants.id, canbus), constants.locationX, constants.locationY));
        }
        m_posY = positiveY;
        m_posX = positiveX;
        m_negY = negativeY;
